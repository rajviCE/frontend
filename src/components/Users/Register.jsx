...
const validationSchema = Yup.object({
  ...
  email: Yup.string()
    .matches(
      /^[a-zA-Z0-9._%+-]+@(gmail\.com|yahoo\.com|facebook\.com|instagram\.com)$/,
      "Only Gmail, Yahoo, Facebook, or Instagram emails are allowed"
    )
...
const RegistrationForm = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema,
    onSubmit: (values) => {
      const { confirmPassword, ...rest } = values; // 🔒 Exclude confirmPassword before sending
      mutateAsync(rest)
        .then((data) => {
          console.log(data);
        })
